CREATE OR REPLACE PACKAGE BODY BOOK_OPERATIONS AS
    ...
    IS
        ...
        v_xml_count     PLS_INTEGER;
        v_json_count    PLS_INTEGER;
        v_matched_count PLS_INTEGER;
    BEGIN

        /*
         * XMLTABLE ile XML kayıt sayısını al.
         */
        SELECT COUNT(*)
        INTO v_xml_count
        FROM XMLTABLE(
        ...
        /*
         * JSON_TABLE ile JSON kayıt sayısını al.
         */
        SELECT COUNT(*)
        INTO v_json_count
        FROM JSON_TABLE(
        ...
        THEN
            RAISE_APPLICATION_ERROR(
                -20004,
                'XML and JSON data cannot be empty'
            );
        END IF;


        /*
         * XML ve JSON kayıt sayıları eşit olmalı.
         */
        IF v_xml_count <> v_json_count THEN
            RAISE_APPLICATION_ERROR(
                -20005,
                'XML and JSON record counts do not match'
            );
        END IF;


        /*
         * XMLTABLE ve JSON_TABLE çıktıları sıra numarası
         * ve içerik üzerinden eşleştirilir.
         *
         * Her kitap yalnızca bir kez INSERT edilir.
         */
        v_matched_count := 0;
        ...
        FOR book_record IN (
            ...
            INNER JOIN (
                ...
                AND j.author_name = x.author_name

                AND j.publisher_name = x.publisher_name
        ...
        LOOP
            ...
            /*
             * Yayınevi mevcutsa ID'sini al,
             * yoksa yeni yayınevi oluştur.
             */
            BEGIN
                SELECT id
                INTO v_publisher_id
        ...
        /*
         * Kayıt sayıları aynı olsa bile içerikler farklı
         * olabilir. Tüm kayıtların eşleşmesi gerekir.
         */
        IF v_matched_count <> v_xml_count THEN
            RAISE_APPLICATION_ERROR(
                -20006,
                'XML and JSON book data do not match'
            );
        END IF;
    ...
    IS
        ...
        v_author_name    AUTHORS.NAME%TYPE;
        v_publisher_name PUBLISHERS.NAME%TYPE;

    BEGIN

        /*
         * Explicit Cursor kullanımı.
         */
        OPEN c_books;
        ...
        /*
         * Java JDBC tarafına SYS_REFCURSOR döndürülür.
         */
