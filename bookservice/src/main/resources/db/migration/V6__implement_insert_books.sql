CREATE OR REPLACE PACKAGE BODY BOOK_OPERATIONS AS
    ...
    BEGIN
        ...
        LOOP
            ...
            v_title :=
                TRIM(REGEXP_SUBSTR(v_book, '[^|]+', 1, 1));

            v_author :=
                TRIM(REGEXP_SUBSTR(v_book, '[^|]+', 1, 2));

            v_publisher :=
                TRIM(REGEXP_SUBSTR(v_book, '[^|]+', 1, 3));
    ...
    BEGIN
        ...
        LOOP
            v_book :=
                REGEXP_SUBSTR(p_raw_data, '[^;]+', 1, v_index);
    ...
    BEGIN
         ...
         * XMLTABLE
         *
         * XML verisindeki her kitap satır satır okunur.
         */
        FOR book_record IN (
        ...
        LOOP
            ...
            /*
             * Yazar daha önce eklenmemişse AUTHORS
             * tablosuna eklenir.
             */
            BEGIN
                SELECT id
            ...
            /*
             * Yayınevi daha önce eklenmemişse
             * PUBLISHERS tablosuna eklenir.
             */
            BEGIN
                SELECT id
                INTO v_publisher_id
         ...
         * ve JSON içindeki kitap kayıtlarını okur.
         *
         * XML ve JSON aynı raw string'den üretildiği için
         * burada tekrar BOOKS tablosuna INSERT yapılmaz.
         * Aksi halde kitaplar iki kez eklenirdi.
         */
        FOR json_record IN (
        ...
        LOOP
            ...
            /*
             * JSON kaydının boş alan içermediğini doğrula.
             */
